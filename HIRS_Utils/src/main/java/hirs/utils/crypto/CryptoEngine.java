...
/** Interface to abstract a cryptographic implementation Current just supports digital signing */
public interface CryptoEngine {
   ...
   * @param algorithm name of the IANA registerd COSE algorithm used to sign or verify a signature
   *     if not specified by the X509 certificate or public key
   * @return true if the key was successfully loaded
   * @throws Exception if an error occurred when loading or processing the key
   */
  public boolean loadPrivateKey(String pathToKey, X509Certificate cert, String algorithm)
      throws Exception;
  ...
  /**
   * Method to verify the signature on signed data using an X.509 Certificate. Either an X.509 or
   * public key must be provided
   *
   * @param signCert A valid x.509 certificate used to verify the signed data, can be null is
   *     publicKey is provided
   * @param publicKey Optional public key used to verify the signed data, can be null is cert is
   *     provided
   * @param algorithm optional string holding the COSE defined signature algorithm.
   ...
   * @throws Exception if an error occurred when processing the with the certificate or signed data
   */
  //  public boolean verify(PublicKey publicKey, byte[] data, byte[] signatureData) throws
  // Exception;

  /**
   * Retrieves Public Key from a KeyPair or Certificate. note that loadPrivateKey may load a public
   * key as well.
   *
