...
/**
 * Class to translate algorithm identifiers whose names differ between the following specs: TCG (TCG
 * Algorithm Registry): https://trustedcomputinggroup.org/resource/tcg-algorithm-registry/ Table 3 —
 * Definition of (UINT16) TPM_ALG_ID Constants XML (XML Security Algorithm Cross-Reference):
 * https://www.w3.org/TR/xmlsec-algorithms/ RFC6931 (Additional XML Security Uniform Resource
 * Identifiers (URIs)): https://datatracker.ietf.org/doc/rfc6931/ CoSwid (Named Information Hash
 * Algorithm Registry): from https://www.rfc-editor.org/rfc/rfc9393.html, points to:
 * https://www.iana.org/assignments/named-information/named-information.xhtml COSE (COSE Header
 * Algorithm Parameters): https://www.iana.org/assignments/cose/cose.xhtml#algorithms Notes: * This
 * class only includes algorithms that pertain to asymmetric signature algorithms and hash
 * algorithms. * This class only includes algorithms that are listed in the TCG Registry, since RIMs
 * are defined by the TCG and the spec states that the RIM MUST use a TCG-defined algorithm.
 */
public class AlgorithmsIds {
  ...
  /*
   * Algorithm Family (e.g. "ECC", "RSA", etc)
   */
  public static final int FAMILY_ALG = 6;

  /**
   * Array that holds the human-readable name of the spec, in the same column order as the
   * subsequent tables
   */
  private static final String[][] algTablesSpecColumns = {
  ...
  private static final String[][] sigAlgorithms = {
    ...
    {
      "TPM_ALG_RSAPSS*TPM_ALG_SHA384",
      "",
      "",
      "PS384",
      "RSA with PSS padding and SHA384",
      "TBD",
      "RSA"
    },
    {
      "TPM_ALG_RSAPSS*TPM_ALG_SHA512",
      "",
      "",
      "PS512",
      "RSA with PSS padding and SHA512",
      "TBD",
      "RSA"
    }
  };
  ...
  /** Default constructor */
  public AlgorithmsIds() {}

  /**
   * Searches algorithm array for match to original spec's alg string, translates that to desired
   * spec alg name
   *
   ...
   */
  public static String translateAlgId(
      String algType, int originalSpec, String originalAlg, int newSpec)
      throws NoSuchAlgorithmException {

    String newAlgId = "";

    if ((newSpec != SPEC_TCG_ALG)
        && (newSpec != SPEC_XML_ALG)
        && (newSpec != SPEC_COSWID_ALG)
        && (newSpec != SPEC_COSE_ALG)
        && (newSpec != SPEC_X509_ALG)) {
    ...
    if (algIdRow >= 0) {
      if (algType.compareTo(ALG_TYPE_HASH) == 0) {
        newAlgId = hashAlgorithms[algIdRow][newSpec];
      } else if (algType.compareTo(ALG_TYPE_SIG) == 0) {
        newAlgId = sigAlgorithms[algIdRow][newSpec];
      }
      ...
      if (newAlgId.compareTo("") == 0) {
        throw new NoSuchElementException(
            "Algorithm "
                + algType
                + " from "
                + algTablesSpecColumns[0][originalSpec]
                + " spec is not defined in "
                + algTablesSpecColumns[0][newSpec]
                + " spec");
      }
    }
  ...
  /**
   * Searches algorithm array for match to spec's alg string, returns true if found
   *
   * @param algType type of algorithm (hash, signature)
   ...
   */
  public static boolean isValid(String algType, int spec, String alg)
      throws NoSuchAlgorithmException {
    return findAlgId(algType, spec, alg) >= 0;
  }

  /**
   * Searches algorithm array for match to spec's alg string, returns row in array where found
   *
   * @param algType type of algorithm (hash, signature)
   ...
   */
  private static int findAlgId(String algType, int spec, String alg)
      throws NoSuchAlgorithmException {

    int index = -1;

    if ((spec != SPEC_TCG_ALG)
        && (spec != SPEC_XML_ALG)
        && (spec != SPEC_COSWID_ALG)
        && (spec != SPEC_COSE_ALG)
        && (spec != SPEC_X509_ALG)) {
    ...
    if (algType.compareTo(ALG_TYPE_HASH) == 0) {
      for (int i = 0; i < hashAlgorithms.length; i++) {
        if (alg.compareTo(hashAlgorithms[i][spec]) == 0) index = i;
      }
    } else if (algType.compareTo(ALG_TYPE_SIG) == 0) {
      ...
      for (int i = 0; i < sigAlgorithms.length; i++) {
        if (alg.compareTo(sigAlgorithms[i][spec]) == 0) index = i;
      }
    } else {
    ...
    if (index == -1) {
      throw new NoSuchElementException(
          "Algorithm " + algType + " is not defined in " + algTablesSpecColumns[0][spec] + " spec");
    }
    return index;
  }
   ...
   */
  public static String toString(String algType, int originalSpec, String originalAlg, int newSpec)
      throws NoSuchAlgorithmException {
    ...
    return "Original specification: "
        + algTablesSpecColumns[0][originalSpec]
        + " ("
        + algTablesSpecColumns[1][originalSpec]
        + ")\nOriginal "
        + algType
        + " algorithm: "
        + originalAlg
        ...
        + algTablesSpecColumns[0][newSpec]
        + " ("
        + algTablesSpecColumns[1][newSpec]
        + ")\nNew "
        + algType
        + " algorithm: "
        + newAlg
  ...
  public static String algFamily(int spec, String alg) throws NoSuchAlgorithmException {
    int row = findAlgId(ALG_TYPE_SIG, spec, alg);
    return sigAlgorithms[row][FAMILY_ALG];
  }

  public static boolean isEcc(int spec, String alg) throws NoSuchAlgorithmException {
    String algFamily = algFamily(spec, alg);
    if (algFamily.compareToIgnoreCase("ECC") == 0) return true;
    else return false;
  }

  public static boolean isRsa(int spec, String alg) throws NoSuchAlgorithmException {
    String algFamily = algFamily(spec, alg);
    if (algFamily.compareToIgnoreCase("RSA") == 0) return true;
    else return false;
  }
}
