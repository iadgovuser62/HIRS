...
public class DefaultCrypto implements CryptoEngine {
  ...
  @Setter @Getter PublicKey publicKey = null;
  @Setter @Getter String algorithm = "";
  @Setter @Getter String kid = "";

  /**
   * Default Crypto Constructor Used as a default mechanism for providing cryptographic operations
   */
  public DefaultCrypto() {}

  /**
   * This method extracts the private key from a file. Supports PEM and JSon Web Keys. Json Web Keys
   * are used to validate external test patterns. Json Web Keys are expected to have parameters for
   * KID and Algorithm. Will load in a public key from a JSON Web key if present. Both PKCS1 and PSS
   * formats are handled for RSA. ECDSA keys are supported. Algorithm argument is present to allow
   * handling of multiple encryption algorithms, but for now it is always RSA.
   *
   * @param pathTokey
   * @return
   */
  public boolean loadPrivateKey(String pathTokey, X509Certificate cert, String sigHashAlg)
      throws Exception {
    String alg, signAlg = "";
  ...
  /**
   * Helper method to lookup a COSE algoritm used by this class from an x509 certificate specified
   * algorithm
   *
   * @param cert a properly formatted x509 certificate
   * @return A IANA published COSE algorithm description
   * @throws NoSuchAlgorithmException if the algorithm is not found in the x509 certificate
   */
  private String getAlgorithmFromCert(X509Certificate cert) throws NoSuchAlgorithmException {
    ...
    String alg =
        AlgorithmsIds.translateAlgId(
            AlgorithmsIds.ALG_TYPE_SIG,
            AlgorithmsIds.SPEC_X509_ALG,
            x509Alg,
  ...
  /**
   * Convert COSE defined algorithm Name to a Java.Security equivalent algorithm string
   *
   * @param coseAlg
  ...
  /**
   * Verifies a signature on a COSE object using a public key vs an X.509 certificate Note that the
   * signature is expected to be in raw (IEEE)
   *
  ...
  public boolean verify(
      X509Certificate cert, PublicKey pk, String sigAlgorithm, byte[] data, byte[] signatureData)
      throws Exception {
    Signature signature = null;
  ...
  /**
   * Helper method to load Java signature instances Supports RSA PKCS1, RSA PSS, and ECDSA
   * signatures.
   *
   * @param alg
   * @return
  ...
  /**
   * Converts the signature block (from COSE) to a DER encoded format used by Java.security
   *
   * @param signature
   * @return
   */
  private byte[] derEncodeRawSignature(byte[] signature) throws IOException {
    ...
    if (CoseAlgorithm.isEcdsaName(algorithm)) {
      if (signature.length >= 94 && signature.length <= 98) n = 48;
      else if (signature.length >= 130 && signature.length <= 134) n = 66;
      ...
      DERSequence sequence =
          new DERSequence(new ASN1Encodable[] {new ASN1Integer(r), new ASN1Integer(s)});
  ...
  /**
   * Removes the Der Encoding from the signature data to produce a "Raw" Signature. Result should be
   * conformant with IEEE P1363
   *
  ...
  private byte[] removeDerFromSignature(byte[] signatureBytes) throws IOException {
    ...
    if (CoseAlgorithm.isEcdsaName(algorithm)) {
      try (ASN1InputStream asn1InputStream = new ASN1InputStream(signatureBytes)) {
        ...
        // Copy individual components into the final array but account for missing leading 0's
        ...
        if (rOffset == 1) rawSignature[0] = 0;
        if (sOffset == 1) rawSignature[rLength + rOffset] = 0;
