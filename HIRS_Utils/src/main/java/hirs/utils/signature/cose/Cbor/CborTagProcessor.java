...
public class CborTagProcessor {
    ...
    boolean isComid = false;
    boolean isCotl = false;
    boolean isOid = false;
    String oid = "";
    String tagLabel = "";
    int tagId = 0;
    ...
    private static int BER_OID_TAG = 0x111;    //object identifier tag per [RFC9090]
     ...
     * This method checks for a Cbor Coswid tag and strips it off if found
     ...
     * Currently only the COSE, Coswid, and CoRim tags are being processed. ALl other tags will result in an exception
    ...
    public CborTagProcessor(byte[] taggedData) {
        ...
        // process per table 7 of rfc 8489ofor integers only (gets to expansive for rarely used options)
        if (tagInfo < 0x18) tagId = tagInfo; // values 0 to 0x17
        else if (tagInfo == 0x18) tagId = (int) taggedData[1];
        ...
        //System.out.println("Tag is " + Integer.toUnsignedString(tagId));
    ...
    /**
     * Helper method to convert an array of bytes to a {@link UUID} object. This works by reconstructing the UUID from
     * the most-significant bits (MSB) and least-significant bits (LSB) contained in two 64-bit components,
     * respectively.
     *
    ...
    /**
     * Convenience method to process a {@link CBORTaggedItem} and associated tag number. The output is contextual, and
     * will require one to typically check the associated class, such as in the following pattern:
     * <p>
     ...
     */
    public static Optional<Object> process(CBORTaggedItem taggedItem) {
        if (taggedItem.getTagNumber().intValue() == 0x20) { // URI
        ...
        } else if (taggedItem.getTagNumber().intValue() == 0x25) { // UUID
        ...
        } else if (taggedItem.getTagNumber().intValue() == 0x6F) { // OID
        ...
        else if (taggedItem.getTagNumber().intValue() == 0x230) { // tagged-bytes
        ...
        else if (taggedItem.getTagNumber().intValue() == 0x233) { // tagged-masked-raw-value
        ...
        else if (taggedItem.getTagNumber().intValue() == 0x258) { // tagged-implementation-id-type
        ...
        else if (taggedItem.getTagNumber().intValue() == 0x259) { // arm-swcomp-id
