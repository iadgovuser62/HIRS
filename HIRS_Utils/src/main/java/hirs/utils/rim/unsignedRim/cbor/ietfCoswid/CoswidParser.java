...
public class CoswidParser {
    ...
    private static int COSWID_MAP = 0;
    private static int ENTITY_MAP =  CoswidItems.ENTITY_INT;

    /* *
     * CBOR parser that decodes rfc 9393 based object https://datatracker.ietf.org/doc/rfc9393/.
     * @param cborData reference to a byte array holding the CBOR-encoded object
     */
    public CoswidParser() {
    }
     ...
     */
    public CoswidParser(byte[] cborData) throws IOException {
     ...
     */
    protected void initParser (byte[] cborData) {
        ...
        try {
            ...
            if (rootNode.path(Integer.toString(CoswidItems.TAG_ID_INT)).binaryValue() == null) {
                ...
                UUID tagUuid = convertBytesToUUID(tagBytes);
            ...
            coswid.setActivationStatus(parseString(CoswidItems.SOFTWARE_META_INT,CoswidItems.ACTIVATION_STATUS_INT));
            coswid.setColloquialVersion(parseString(CoswidItems.SOFTWARE_META_INT,CoswidItems.COLLOQUIAL_VERSION_INT));
            ...
            coswid.setProductFamily(parseString(CoswidItems.SOFTWARE_META_INT, CoswidItems.PRODUCT_FAMILY_INT));
     ...
     */
    private void processRole(JsonNode node) {
        ...
        if (node != null) {
            ...
            if (node.isArray()) {
                for (i=0; i< node.size(); i++) {
                    ...
                    coswid.roleCoswid.add(value);                           // CHECK THIS SYNTAX
                    nonpayloadParsedDataOneline += " " + value;
                    nonpayloadParsedDataPretty += "            " + value + "\n";
                }
            }
            else {                                                          // CHECK IF THIS CASE IS EVER HIT
                key = node.asInt();
                value = roleLookup(key);
     ...
     */
    private String roleLookup (int index) {
        String role = "";
        switch (index) {
     ...
     */
    private void processPayload(JsonNode node) {
     ...
     */
    private void processPayloadDirectory(JsonNode directoryNode) {
     ...
     */
    private void processPayloadFile(JsonNode fileNode) {
        ...
        // TODO process the measurement type
        ...
        while (fields.hasNext()) {
            ...
            if ( index.compareTo(CoswidItems.HASH_STR)==0) {
                ...
                //measurement.setAlgInt((Integer) hashInfo[0]);
            }
            else {
                switch (index) {
                    case CoswidItems.FS_NAME_STR:
                        ...
                        break;
                    //case CoswidItems.SIZE_STR: (possibly use in future)
                    //case CoswidItems.FILE_VERSION_STR: (possibly use in future)
                }
                payloadParsedDataOneline += " " + index + " => " + field.getValue();
                payloadParsedDataPretty += "         "  + index + " : " + field.getValue() + "\n";
            }
     ...
     */
    private Object[] processPayloadHash(JsonNode node) {
    ...
    /**
     * Converts a tagid  which is a "16-byte binary string" (per rfc 9393) into a UUID
     * @param bytes tagid bytes to convert to UUID
     * @return UUID object holding the tagid
     */
     ...
     */
    private void buildString(int key, String item) {
     ...
     */
    private byte[] checkForTag(byte[] coswidData) {
     ...
     */
    private String parseString(int map, int coswidItem){
        String itemString = Integer.toString(coswidItem), mapString = Integer.toString(map), referenceVal = "";
        if (map == COSWID_MAP) {
     ...
     */
    private boolean parseBoolean(int map, int coswidItem){
        String itemString = Integer.toString(coswidItem), mapString = Integer.toString(map);
