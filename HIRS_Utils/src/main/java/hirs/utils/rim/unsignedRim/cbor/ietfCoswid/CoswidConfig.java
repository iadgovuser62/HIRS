...
public class CoswidConfig extends Coswid {
     ...
     */
    public CoswidConfig(String filename) throws IOException{
        ...
        try {
           ...
           // e.printStackTrace();
            throw new RuntimeException("Error processing Coswid RIM configuration file named " + filename+ ": " + e.getMessage(), e);
        }
    }
     ...
     * @param initNode
     */
        protected void init(JsonNode initNode) {
            ...
            // Meta fields
            colloquialVersion = rootNode.path(CoswidItems.SOFTWARE_META_STR).path(CoswidItems.COLLOQUIAL_VERSION_STR).asText();
            ...
            description = rootNode.path(CoswidItems.SOFTWARE_META_STR).path(CoswidItems.DESCRIPTION_STR).asText();
            persistentId = rootNode.path(CoswidItems.SOFTWARE_META_STR).path(CoswidItems.PERSISTENT_ID_STR).asText();
            productFamily = rootNode.path(CoswidItems.SOFTWARE_META_STR).path(CoswidItems.PRODUCT_FAMILY_STR).asText();
