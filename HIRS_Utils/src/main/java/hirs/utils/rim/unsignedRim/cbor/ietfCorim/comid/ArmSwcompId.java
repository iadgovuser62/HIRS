...
public class ArmSwcompId {
     ...
     */
    public ArmSwcompId(CBORItem mkeyItem) {
        ...
        for (var currItem : list) {
            ...
            if (currType != null) {
                switch (currType) {
                    case MEASUREMENT_TYPE -> measurementType = (String) currItem.getValue().parse();
                    case VERSION -> version = (String) currItem.getValue().parse();
                    case SIGNER_ID -> signerId = (byte[]) currItem.getValue().parse();
                }
            }
        }
    }
