...
/**
 * Defines a {@code triples-map}, which contains CoMID <i>triples</i>. Triples are records that link various assertions
 * with security features (such as cryptographic keys, etc.).
 * <p>
...
public class TriplesMap {
    /** Corresponds to a list of {@code reference-triple-record} values. A <i>reference values triple</i> in this list
     * contains reference measurements or claims pertaining to a target environment.
     */
    List<ReferenceTripleRecord> referenceTriples;
     ...
     */
    public TriplesMap(CBORItem triplesMap) {
                ...
                switch (currVal) {
                    case REFERENCE_TRIPLES -> {
                        ...
                        for (int j = 0; j < currRefTriples.size(); j++) {
                            referenceTriples.add(new ReferenceTripleRecord(currItem.getValue())); // Reference triples
                        }
                    }
