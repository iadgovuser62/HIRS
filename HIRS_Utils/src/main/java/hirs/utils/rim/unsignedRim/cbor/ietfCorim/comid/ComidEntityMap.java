...
/**
 *  Class that processes a {@code comid-entity-map} containing hash information relevant to CoMID measurements. A
 *  <i>CoMID Entity</i> contains information about an organization responsible for CoMID contents. See Section 5.1.2
...
public class ComidEntityMap {
    /** Corresponds to the {@code entity-name} field. The name of the entity responsible for the role's actions.*/
    String entityName;
    /** Corresponds to the {@code reg-id} field. A URI associated with the owner organization for the entity name.*/
    URI regId;
    /** Corresponds to the {@code role-type-choice} field. Defines a list of roles that the entity is claiming.*/
    List<ComidRoleTypeChoice> roleTypeChoice;
     ...
     */
    public ComidEntityMap(CBORItem comidEntityMap) {
        ...
        for (var currItem : list) {
            ...
            if (currVal != null) {
                switch (currVal) {
                    case ENTITY_NAME -> entityName = (String)currItem.getValue().parse(); // Entity name
                    case REG_ID -> {
                    ...
                    case ROLE -> {
                        ...
                        if (roleObj instanceof List<?> roleTypeList) {
                            for (var currRole : roleTypeList) {
                                ...
                                if (currRoleVal != null) roleTypeChoice.add(currRoleVal);
                            }
                        }
                    }
                }
