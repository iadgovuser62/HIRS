...
public class PcClientRim extends SwidTagGateway implements GenericRim {
    ...
    public boolean validate (String verifyFile ,String certificateFile, String rimel, String trustStore) throws IOException { boolean valid= false;
        ...
        rim = validateSwidtagSchema(removeXMLWhitespace(new StreamSource(
                new ByteArrayInputStream(rimBytes))));

        NodeList si =  rim.getElementsByTagNameNS(SwidTagConstants.SWIDTAG_NAMESPACE, SwidTagConstants.SOFTWARE_IDENTITY);
        ...
        // Process Meta Data
        NodeList metaNode =  rim.getElementsByTagNameNS(SwidTagConstants.SWIDTAG_NAMESPACE, SwidTagConstants.META);
        Element mElement = (Element)metaNode.item(0);
        manufacturer =  mElement.getAttributeNS(SwidTagConstants.TCG_NS, SwidTagConstants.PLATFORM_MANUFACTURER_STR);
      ...
      NodeList files = rim.getElementsByTagNameNS(SwidTagConstants.SWIDTAG_NAMESPACE, SwidTagConstants.FILE);
      // Make a measurement for each Hash item and add it to the list of measurements
      for (int count=0; count<files.getLength(); count++ ) {
          Element file = (Element) files.item(count);
          digest = file.getAttributeNS(SwidTagConstants.SHA_256_HASH.getNamespaceURI(), SwidTagConstants.HASH);
