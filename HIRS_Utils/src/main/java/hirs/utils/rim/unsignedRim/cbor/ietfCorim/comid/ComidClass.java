 ...
 *  of the IETF CoRIM specification. An environment class contains uniquely identifying information that is used by an
 *  {@link EnvironmentMap}.
 */
@Getter
@Setter
...
public class ComidClass {
    ...
    byte[] classIdBytes;
    /** Corresponds to the {@code vendor} field. The entity responsible for choosing values for the other class
     * attributes without naming authority.*/
    String vendor;
    /** Corresponds to the {@code model} field: describes a product, generation, and family.*/
    String model;
    /** Corresponds to the {@code layer} field. Captures the sequence in which the environment exists (contextual).*/
    Integer layer;
     ...
     */
    public ComidClass(CBORItem comidClassItem) {
        ...
        for (var pair : pairs) {
            ...
            if (currVal != null) {
                switch (currVal) {
                    ...
                    case VENDOR -> vendor = ((String)pair.getValue().parse());
                    case MODEL ->  model = ((String)pair.getValue().parse());
                    case LAYER -> layer = ((int)pair.getValue().parse());
                    case INDEX -> index = ((int)pair.getValue().parse());
                }
            }
        }
    }
