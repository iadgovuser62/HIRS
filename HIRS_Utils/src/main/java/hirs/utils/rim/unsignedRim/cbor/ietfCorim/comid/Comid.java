...
/**
 *  Class that processes a <i>CoMID</i>. A CoMID (Concise Module Identifier) contains various attributes pertaining to
 *  hardware, firmware, or modules on a device. See Section 5 of the IETF CoRIM specification for further information.
 *  <p>
...
public class Comid {
    ...
    TagIdentityMap tagIdentity;
    /** Corresponds to the {@code entities} field. Contains a list of entities responsible for CoMID production.*/
    List<ComidEntityMap> entities;
    /** Corresponds to the {@code linkedTags} field. Contains a list of {@link LinkedTagMap} objects describing
     * CoMID relationships.*/
    List<LinkedTagMap> linkedTags;
    /** Corresponds to the {@code triples} field. Contains a reference to a {@link TriplesMap} object containing
     * <i>triples</i>, which are records that link various assertions with security features (such as cryptographic
     * keys, etc.).*/
    TriplesMap triples;
            ...
            for (var currItem : list) {
                ...
                if (currVal != null) {
                    switch (currVal) {
                        ...
                        case LANGUAGE -> language = Locale.forLanguageTag((String)currItem.getValue().parse());
                        // Tag identity map
                        case TAG_ID -> tagIdentity = new TagIdentityMap(currItem.getValue());
                        ...
                        case TRIPLES_MAP -> triples = new TriplesMap(currItem.getValue()); // Triples
                    }
                }
            }
        } catch (IOException e) {
