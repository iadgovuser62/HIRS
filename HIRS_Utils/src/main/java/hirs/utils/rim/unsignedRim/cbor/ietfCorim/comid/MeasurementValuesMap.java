...
/**
 * A class corresponding to <i>measured values</i>, which are measurements associated with a particular environment. See
 * Section 5.1.4.4.2 of the IETF CoRIM specification.
 */
@Setter
@Getter
...
public class MeasurementValuesMap {
    ...
    ComidSvn svn;
    /** Contains a list of {@code digest} objects for the measured environment, along with the hash algorithm.*/
    List<ComidDigest> digests;
    /** Corresponds to the {@code raw-value} field. A raw value is the actual (non-hashed) value of the element.*/
    RawValue rawValue;
     ...
     */
    public MeasurementValuesMap(CBORItem mValuesMap) {
        ...
        for (var currItem : list) {
            ...
            if (currVal != null) {
                switch (currVal) {
                    case VERSION_MAP -> version = new VersionMap(currItem.getValue()); // Version map
                    case SVN -> {
                    ...
                    case RAW_VALUE -> {
                        // $raw-value-type-choice can either be of type tagged-bytes or tagged-masked-raw-value
                        // See section 5.1.4.1.4.6 of the IETF CoRIM specification
                        if (rawValue == null) rawValue = new RawValue();
                        CborTagProcessor.process((CBORTaggedItem) currItem.getValue()).ifPresent(parsedRawValue -> {
                    ...
                    case RAW_VALUE_MASK -> {
                        ...
                        // compatibility
                        if (rawValue == null) rawValue = new RawValue();
                        rawValue.mask = (byte[]) currItem.getValue().parse();
                    }
                    // Note: no standard Java library class exists for MAC addresses, so byte[] is used instead
                    case MAC_ADDR -> macAddr = (byte[]) currItem.getValue().parse();
                    case IP_ADDR -> {
                        try {
                    ...
                    case SERIAL_NUMBER -> serialNumber = (String) currItem.getValue().parse();
                    case UEID -> ueid = (byte[])currItem.getValue().parse(); // See draft-ietf-rats-eat, section 4.2.1.2
                    case UUID -> uuid = UUID.fromString((String)currItem.getValue().parse());
                    case NAME -> name = (String)currItem.getValue().parse();
