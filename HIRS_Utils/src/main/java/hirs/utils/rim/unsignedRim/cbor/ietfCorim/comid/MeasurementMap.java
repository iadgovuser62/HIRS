...
public class MeasurementMap {
    /** Corresponds to the optional {@code mkey} (measurement key) field. If no mkey is present, the measurement is
     * considered <i>anonymous</i>.<p>Represents the {@code tagged-oid-type} choice.*/
    @JsonProperty("mkey (oid-type)")
    Oid mkeyOid;
    /** Corresponds to the optional {@code mkey} (measurement key) field. If no mkey is present, the measurement is
     * considered <i>anonymous</i>.<p>Represents the {@code tagged-uuid-type} choice.*/
    @JsonProperty("mkey (uuid-type)")
    UUID mkeyUUID;
    /** Corresponds to the optional {@code mkey} (measurement key) field. If no mkey is present, the measurement is
     * considered <i>anonymous</i>.<p>Represents the {@code tstr} choice.*/
    @JsonProperty("mkey (tstr)")
    String mkeyStr;
    /** Corresponds to the optional {@code mkey} (measurement key) field. If no mkey is present, the measurement is
     * considered <i>anonymous</i>.<p>Represents the {@code uint} choice.*/
    @JsonProperty("mkey (uint)")
    Integer mkeyInt;
    /** Corresponds to an {@link ArmSwcompId} object, which is an extension found in the IETF Arm CCA Endorsements
     * specification.*/
    @JsonProperty("mkey (Arm CCA extension)")
    ArmSwcompId mkeyArmSwcompId;
    /** Corresponds to an {@code mval}, or measurement associated with the environment.*/
    MeasurementValuesMap mval;
     ...
     */
    public MeasurementMap(CBORItem measurementMap) {
            ...
            if (currVal != null) {
                switch (currVal) {
                    case MKEY -> {
                        ...
                        /* $measured-element-type-choice can be either a tagged-oid-type, tagged-uuid-type, uint, or tstr
                         * See section 5.1.4.1.4.1 in the IETF CoRIM specification
                         *
                         * Additionally, the IETF draft-ydb-rats-cca-endorsements extends $measured-element-type-choice
                         * to add tagged-arm-swcomp-id (handled below if present)
                         */
                        if (currMkey instanceof CBORTaggedItem) {
                            // Handle tagged item: could be extension as seen in IETF draft-ydb-rats-cca-endorsements
                    ...
                    case MVAL -> mval = new MeasurementValuesMap(currPair.getValue());
