...
/**
 * An object representing a {@code tag-identity-map} from Section 5.1.1 of the IETF CoRIM specification. Contains
 * unique identifying information for the CoMID.
 */
@Getter
@Setter
...
public class TagIdentityMap {
    /** Corresponds to the {@code tag-id} field, representing a globally unique identifier for the CoMID. Represents
     * the {@code tstr} choice.*/
    @JsonProperty("tag-id (tstr)")
    String tagIdStr;
    /** Corresponds to the {@code tag-id} field, representing a globally unique identifier for the CoMID. Represents
     * the {@code uuid-type} choice.*/
    @JsonProperty("tag-id (uuid-type)")
    UUID tagIdUUID;
    /** Corresponds to the {@code tag-version} field. The tag version is a positive numeric value corresponding to the
     * release revision of the tag.*/
    Integer tagVersion;
     ...
     */
    public TagIdentityMap(CBORItem tagIdentityMap) {
                ...
                switch (currVal) {
                    case TAG_ID -> {
                        ...
                        if (tagIdParsed instanceof String) {
                            ...
                            tagIdUUID = CborTagProcessor.bytesToUUID((byte[])currItem.getValue().parse()); // uuid-type
                        }
                    }
                    // Tag version
                    case TAG_VERSION -> tagVersion = (int)currItem.getValue().parse();
                }
            }
        }
    }
