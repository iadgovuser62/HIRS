 ...
 * Uses the indexes to convert the integer key into the text values published in rfc 9393 Section 2.3 "The concise-swid-tag Map"
 */
public class CoswidBuilder extends Coswid {
    ...
    public CoswidBuilder(CoswidConfig conf) {
        config = conf;
        // Software
     ...
     * @param fileName
     */
    public void createCoswidData(String fileName) throws IOException {
     ...
     */
    public ByteArrayOutputStream initCoswid(ByteArrayOutputStream out) {
        try {
            ...
            cborGen.writeEndObject();
            // Meta - Create Array and add Meta elements from CoSwid, NIST IR 8060, TCG PC Client RIM, and TCG Component RIM
     ...
     * @return ByteArrayOutputStream that contains the encoded Coswid data
     */
    protected ByteArrayOutputStream completeCoswid(ByteArrayOutputStream out) {
        try {
     ...
     */
    protected void addStringAttribute(String attribute, int fieldItem, ByteArrayOutputStream out) {
        // byte[] value = attribute.getBytes(StandardCharsets.UTF_8);
        if (attribute == null) return;      // Skip if variable was null
        if (attribute.isEmpty()) return;      // Skip if variable was empty
        try {
     ...
     * @param out       ByteArrayOutputStream that holds the encoded Coswid object
     */
    protected void addIntAttribute(int attribute, int fieldItem, ByteArrayOutputStream out) {
        try {
            cborGen.writeFieldId(fieldItem); // index
     ...
     * @param out       ByteArrayOutputStream that holds the encoded Coswid object
     */
    protected void addBooleanAttribute(boolean attribute, int fieldItem, ByteArrayOutputStream out) {
        try {
            cborGen.writeFieldId(fieldItem); // index
     ...
     */
    protected void addTagId(String tagId, ByteArrayOutputStream out) {
     ...
     */
    protected void addRoles(String roles, int roleIndex, ByteArrayOutputStream out) {
     ...
     * @return
     */
    protected byte[] convertUUIDtoBytes(String guid) {
     ...
     */
    protected int roleLookup(String role) {
        int roleValue = 0;
        switch (role) {
            case "tag-creator":
                roleValue = 1;
                break;
            case "software-creator":
                roleValue = 2;
                break;
            case "aggregator":
                roleValue = 3;
     ...
     * @param out ByteArrayOutputStream that holds the encoded Coswid object
     */
    protected void createPayload(JsonNode payloadNode, ByteArrayOutputStream out) {
     ...
     * @throws IOException
     */
    protected void createDirectory(JsonNode node, ByteArrayOutputStream out) throws IOException {
        //addStringAttribute("Directory", Coswid.COSWID_DIRECTORY_INT, out);
     ...
     * @throws IOException
     */
    protected void createFile(JsonNode node, ByteArrayOutputStream out) throws IOException {
        int i = 0;
        // If multiple files are provided then process one at a time
        if (node.isArray()) {
            cborGen.writeStartArray();
            for (i=0; i< node.size(); i++) {
                createFile(node.get(i), out);
            }
     ...
     * @throws IOException
     */
    protected void createFileHash(ByteArrayOutputStream out, String hash, IanaHashAlg alg) throws IOException {
     ...
     * Note in section 8 of RFC 9393 states that Coswid tags should be tagged but that redundant tagging should be avoided
     * @param untaggedCoswid  Coswid Byte array to add the tag to
     */
    public ByteArrayOutputStream addCborTag(ByteArrayOutputStream untaggedCoswid) throws IOException {
        ...
        CBORTaggedItem taggedCbor = new CBORTaggedItem(Coswid.ianaTag, coswidData);
